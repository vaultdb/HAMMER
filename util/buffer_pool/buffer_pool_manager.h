...
namespace vaultdb {
    ...
    public:
        ...
        void initialize(int unpacked_page_size, int num_unpacked_pages, CryptoManager *manager) {}
        ~BufferPoolManager() {};
        ...
        int page_size_bits_; // in emp::Bits
        int page_cnt_;
        ...
        std::vector<emp::Bit> buffer_pool_;
...
namespace vaultdb {
    ...
    public:
        ...
        int page_size_bits_ = 2048; // in emp::Bits, default value
        int page_cnt_;
        ...
        void initialize(int unpacked_page_bits, int unpacked_page_cnt, EmpManager *manager)  {
            page_size_bits_ = unpacked_page_bits;
            page_cnt_ = unpacked_page_cnt;
            emp_manager_ = manager;
            // block size of each packed wire based on unpacked page size
            block_n_ = page_size_bits_ / 128 + (page_size_bits_ % 128 != 0);
            // initialize unpacked page buffer
            buffer_pool_ = std::vector<emp::Bit>(page_size_bits_ * page_cnt_, emp::Bit(0));
        ...
        emp::Bit *getPagePtr(PageId &pid) {
            loadPage(pid);
            return buffer_pool_.data() + position_map_.at(pid).slot_id_ * page_size_bits_;
        }
        ...
        void removeTable(int target_table_id) {
            for (auto pos = position_map_.begin(); pos != position_map_.end(); ++pos) {
                if (pos->first.table_id_ == target_table_id) {
                    ...
                    // TODO: add flushing in destructor of PackedColumnTable when we persist it to disk
                    // flushPage(p);
                    reverse_position_map_.erase(slot);
                    // position_map_.erase(p);
                }
            }
            // signal that the PackedColumnTable is no longer available
    ...
    private:
        ...
        std::map<int, QueryTable<Bit> *> table_catalog_;
