...
namespace vaultdb {
    ...
    public:
        ...
        static vector<size_t> checkSwapIOUtilization();

        static size_t checkMemoryAndSwapUtilization();

        static void checkDiskIOUtilization();

        static std::vector<unsigned long long> getDiskIOUtilization();

        static uint64_t getCPUTime();

        static double getCPUUsage(uint64_t start_cpu_time, uint64_t end_cpu_time, int elapsed_time);

        static std::map<std::string, unsigned long long> getNetworkIO();

        static size_t getMemoryBandwidth();
        static size_t getL3CacheMisses();
        static size_t getCPUStallCycles();
        static double getIPC();
    ...
    static std::string getStackTrace();
        ...
        static string getTestParameters() {
            ...
            output << "Crypto mode: " << CryptoManager::cryptoModeString(s.crypto_mode_) << ", storage mode: " << getStorageModeStr(s.storageModel()) << endl;
