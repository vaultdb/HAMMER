...
void encodeTable(string table_name) {
    ...
    if(conf_.buffer_pool_enabled_) {
        BufferedColumnTable * secure_table = (BufferedColumnTable *) plain_table->secretShare();

        std::this_thread::sleep_for(std::chrono::seconds(1));

        conf_.bpm_.flushTable(secure_table->table_id_);

        std::this_thread::sleep_for(std::chrono::seconds(1));

        // rename the file to table name
        string rename_cmd = "mv " + secure_table->secret_shares_path_ + " " + secret_shares_file;
        Utilities::runCommand(rename_cmd);
        ...
        int current_bit_size_on_disk = party_ == 1 ? empBitSizesInPhysicalBytes::evaluator_disk_size_
                                                   : (party_ == 10086 ? 1
                                                                      : empBitSizesInPhysicalBytes::garbler_disk_size_);
...
int main(int argc, char **argv) {
    ...
    // validate it
    if(VALIDATE) {
        conf_.initializeOutsourcedSecretShareDb(dst_root_, temp_data_path_);
