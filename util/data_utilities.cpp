...
std::unique_ptr<PlainColumnTable> DataUtilities::getQueryColumnTable(const std::string &db_name, const std::string &sql) {
    PsqlDataProvider dataProvider;
    return dataProvider.getQueryColumnTable(db_name, sql);
}
...
size_t DataUtilities::getTupleCount(const string &db_name, const string &sql, bool has_dummy_tag) {
    ...
    size_t cnt = res->getField(0,0).getValue<int64_t>();
    delete res;
    return cnt;
...
SecureTable *DataUtilities::readSecretSharedInput(const string &secret_shares_input, const QuerySchema &plain_schema, const int & limit) {
    ...
    return readSecretSharedInput(src_data, plain_schema, limit);
}

SecureTable * DataUtilities::readSecretSharedInput(vector<int8_t> & src_data, const QuerySchema &plain_schema,
    const int &limit) {
