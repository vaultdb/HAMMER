...
#include "openfhe.h"
    using CryptoContext_ = lbcrypto::CryptoContextImpl<lbcrypto::DCRTPoly>;
    using Ciphertext_ = lbcrypto::Ciphertext<lbcrypto::DCRTPoly>;
    using Plaintext_ = lbcrypto::PlaintextImpl;
    using PrivateKey_ = lbcrypto::PrivateKeyImpl<lbcrypto::DCRTPoly>;
    using PublicKey_ = lbcrypto::PublicKeyImpl<lbcrypto::DCRTPoly>;
#endif
...
enum class OperatorType {
    ...
    MERGE_JOIN,
    COLUMN_KEYED_NESTED_LOOP_JOIN,
    FHE_KEYED_NESTED_LOOP_JOIN,
    SORT,
    SHRINKWRAP,
    SCALAR_AGGREGATE,
...
struct empBitSizesInPhysicalBytes {
    const static int num_computing_party_ = N;
