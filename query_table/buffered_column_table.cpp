...
std::vector<emp::Bit> BufferedColumnTable::readSecretSharedPageFromDisk(const PageId pid) {
    ...
    size_t reading_tuple_cnt = (remained_tuple_cnt < this->fields_per_page_[col]) ? remained_tuple_cnt : this->fields_per_page_[col];
    ...
    size_t fread_offset = this->serialized_col_bytes_offsets_on_disk_[col] + (int64_t) (pid.page_idx_ * this->fields_per_page_[col] * secure_field.size() * current_emp_bit_size_on_disk_);
...
std::vector<emp::Bit> BufferedColumnTable::readSecretSharedPageFromDisk(const PageId pid, const int tuple_cnt, const QuerySchema &schema, const int src_col, const string &src_data_path) {
    ...
    size_t fread_offset = target_offset + pid.page_idx_ * this->fields_per_page_[col] * secure_field.size() * current_emp_bit_size_on_disk_;
    fseek(fp, fread_offset, SEEK_SET);
...
void BufferedColumnTable::flushPage(const PageId &pid, emp::Bit *bits) {
    ...
    FILE*  fp = fopen(this->secret_shares_path_.c_str(), "rb+");
    size_t write_offset = this->serialized_col_bytes_offsets_on_disk_[col] + (int64_t) (pid.page_idx_ * this->fields_per_page_[col] * this->schema_.getField(col).size() * current_emp_bit_size_on_disk_);
