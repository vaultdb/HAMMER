...
namespace  vaultdb {
    ...
    class QueryTable {
        ...
        virtual void getPage(const PageId &pid, Bit *dst) = 0;
        virtual void flushPage(const PageId &pid, Bit *src) = 0;
        ...
        /// Additive secret sharing (XOR) for FHE–MPC integration.
        /// Each party calls with its share (e.g. R or M-R); both run in sync.
        /// Returns an N-row SecureTable (not concatenated). Uses EMP; override only for plain tables.
        virtual SecureTable *secretShareAdditive(const int & my_party);
       ...
       Integer unpackRow(const int & row, const int & col_cnt=-1) const {
           ...
           auto src = (SecureTable *) this;
            ...
            emp::Bit b = b_field.getValue<emp::Bit>();
            *cursor = b;
            return dst.bits;
        }
        ...
        Integer unpackRow(const int & row, const int & col_cnt, const int & selection_length_bits) const {
            ...
            emp::Bit *write_cursor = dst.bits.data();
         ...
         inline void packRow(const int & row, Integer src) {
           ...
           auto dst = (SecureTable *) this;
             emp::Bit *cursor = src.bits.data();
             emp::Bit *end = src.bits.data() + src.size() - 1; // -1 for dummy tag, do this separately
            int dst_idx = 0;
            while(cursor < end) {
