...
template<typename B>
SecureTable *QueryTable<B>::secretShareAdditive(const int &) {
    throw std::runtime_error("secretShareAdditive not supported for this table type");
}
...
QueryTable<B> *QueryTable<B>::getTable(const size_t &tuple_cnt, const QuerySchema &schema, const SortDefinition &sort_def) {
    ...
    if(s == StorageModel::COLUMN_STORE && conf.buffer_pool_enabled_ && is_same_v<B, emp::Bit>) {
