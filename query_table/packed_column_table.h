...
public:
    ...
    FILE* packed_pages_file_; // file pointer for read and write
    map<int, long> ordinal_offsets_; // offsets for each column in the packed pages file
    ...
    PackedColumnTable(const size_t &tuple_cnt, const QuerySchema &schema, const SortDefinition &sort_def = SortDefinition()) : QueryTable<Bit>(tuple_cnt, schema, sort_def), manager_(SystemConfiguration::getInstance().emp_manager_) {
        ...
        vector<Bit> ones(bpm_.page_size_bits_,Bit(true));

        one_wire_ = OMPCPackedWire(bpm_.block_n_);
        manager_->pack(ones.data(), reinterpret_cast<Bit *>(&one_wire_), bpm_.page_size_bits_);
    ...
    Field<Bit> getField(const int  & row, const int & col)  const override {
        ...
        if(field_blocks == 1) {
            ...
            emp::Bit *read_ptr = bpm_.getPagePtr(pid) + ((row % fields_per_wire_.at(col)) * schema_.getField(col).size());
            return  Field<Bit>::deserialize(schema_.getField(col), (int8_t *) read_ptr);
        }

        vector<Bit> bits(field_blocks * bpm_.page_size_bits_);
        ...
        for(int i = 0; i < field_blocks; ++i) {
           ...
           ++pid.page_idx_;
           write_ptr += bpm_.page_size_bits_;
        }
    ...
    inline void setField(const int  & row, const int & col, const Field<Bit> & f)  override {
        ...
        if(field_blocks == 1) {
            PageId pid(table_id_, col, row /fields_per_wire_.at(col));
            emp::Bit *write_ptr = bpm_.getPagePtr(pid) +   ((row % fields_per_wire_.at(col)) * schema_.getField(col).size());
        ...
        for(int i = 0; i < field_blocks; ++i) {
            Bit *write_ptr = bpm_.getPagePtr(pid);
            memcpy(write_ptr, read_ptr, bpm_.page_size_bits_ * sizeof(Bit));
            bpm_.markDirty(pid);
            ++pid.page_idx_;
            read_ptr += bpm_.page_size_bits_;
        }
    ...
    Bit getDummyTag(const int & row)  const override {
        ...
        emp::Bit *read_ptr = bpm_.getPagePtr(pid) + (row % fields_per_wire_.at(-1));
        return *read_ptr;
    }

    void setDummyTag(const int & row, const Bit & val) override {
        auto pid = bpm_.getPageId(table_id_, -1, row, fields_per_wire_.at(-1));
        emp::Bit *write_ptr = bpm_.getPagePtr(pid) + (row % fields_per_wire_.at(-1));
        *write_ptr = val;
        bpm_.markDirty(pid);
    }
    ...
    void setSchema(const QuerySchema &schema) override {
        ...
        for(int i = 0; i < schema_.getFieldCount(); ++i) {
            ...
            int bits_per_page = bpm_.page_size_bits_;
        ...
        fields_per_wire_[-1] = bpm_.page_size_bits_;

        tuple_size_bytes_ += sizeof(emp::Bit);
        field_sizes_bytes_[-1] = sizeof(emp::Bit);
        ordinal_offsets_ = getOrdinalOffsets(schema_, tuple_cnt_);
    }
    ...
    void cloneColumn(const int & dst_col, const int & dst_row, const QueryTable<Bit> *src, const int & src_col, const int & src_row = 0) override {
        ...
        if ((dst_row == 0) && (src_row == 0) && (tuple_cnt_ >= src->tuple_cnt_))    {
            ...
            bpm_.flushColumn(src_table->table_id_, src_col);
            memcpy(packed_pages_[dst_col].data(), src_table->packed_pages_[src_col].data(), src_table->packed_pages_[src_col].size());
            return;
        }
    ...
    static inline long bytesPerColumn(const QueryFieldDesc & desc, const int & tuple_cnt)  {
        ...
        int bits_per_wire = BufferPoolManager::getInstance().page_size_bits_;
    ...
    static map<int, long>  getOrdinalOffsets(const QuerySchema &schema, const int & tuple_cnt) {
        ...
        for(int i = 0; i < schema.getFieldCount(); ++i) {
            ...
            array_byte_cnt +=  PackedColumnTable::bytesPerColumn(schema.getField(i), tuple_cnt);
        }
    ...
    ~PackedColumnTable() {
        ...
        // close the file
        if (packed_pages_file_ != nullptr) {
            fclose(packed_pages_file_);
        }
...
protected:
    ...
    void appendColumnSetup(const QueryFieldDesc & desc) {
        ...
        int size_threshold = bpm_.page_size_bits_;
        ...
        if(desc.size() / size_threshold > 0) {
            ...
            wires_per_field_[ordinal] = packed_wires;
        }
        else {
