...
SecureSqlInput::SecureSqlInput(const string &db, const string &sql, const bool &dummy_tag, const int &input_party, const size_t & input_tuple_limit, const SortDefinition &sort_def) :
    ...
    if (input_tuple_limit != 0 && input_tuple_limit < plain_input_->tuple_cnt_)
...
void SecureSqlInput::runQuery() {
    ...
    CryptoMode crypto_mode = SystemConfiguration::getInstance().crypto_mode_;

    if(input_party_ > 0) {
        has_input = (party == input_party_);
    }
    if(crypto_mode == CryptoMode::EMP_ZK_MODE && party == 2) {
        has_input = false;
    }

    // for now we only have input from TP
    if(crypto_mode == CryptoMode::EMP_OUTSOURCED && input_party_ == 1 && party == emp::TP) {
        has_input = true;
    }
