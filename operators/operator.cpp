...
QueryTable<B> *Operator<B>::run() {
    ...
    if(std::is_same_v<B, Bit> && this->getOperatorId() > -2) {
        ...
        if (gate_cnt_ > 0  && this->getOperatorId() >= -1 && output_schema_.size() > 0) {
        ...
        log->write("Operator #" + std::to_string(this->getOperatorId()) + " " + getTypeString() +
                   " ran for " + std::to_string(runtime_ms_) + " ms, " +
                   " output cardinality: " + std::to_string(output_->tuple_cnt_) +
                   " true output cardinality: " + std::to_string(output_->getTrueTupleCount()) +
...
std::string Operator<B>::printTree() const {
    auto plan_str =  printHelper("");
    if (std::is_same_v<B, Bit>) {
        auto cost_est = OperatorCostModel::planCost((SecureOperator *) this);
        plan_str += "Estimated cost: " + std::to_string(cost_est) + "\n";
    }
...
void Operator<B>::detectCTE() {
    ...
    for (const auto& [id, count] : op_counts) {
        if (count > 1) {
            this->setCteFlag(true);
            return;
        }
    }
    this->setCteFlag(false);
}


// Member function: starts the canonicalization process.
template<typename B>
Operator<B>* Operator<B>::canonicalized() {
    std::unordered_map<int, Operator<B>*> canonicalMap;
    return canonicalizedHelper(canonicalMap);
}

// Private helper: recursively canonicalizes the tree.
template<typename B>
Operator<B>* Operator<B>::canonicalizedHelper(std::unordered_map<int, Operator<B>*>& canonicalMap) {
    int id = this->getOperatorId();
    // If this operator id has already been seen, return the canonical instance.
    if (canonicalMap.find(id) != canonicalMap.end()) {
        return canonicalMap[id];
    }

    // Mark this operator as the canonical instance.
    canonicalMap[id] = this;

    // Recursively process the left child (index 0)
    if (this->getChild(0) != nullptr) {
        this->setChild(this->getChild(0)->canonicalizedHelper(canonicalMap), 0);
    }

    // Recursively process the right child (index 1)
    if (this->getChild(1) != nullptr) {
        this->setChild(this->getChild(1)->canonicalizedHelper(canonicalMap), 1);
    }
