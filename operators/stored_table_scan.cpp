...
QueryTable<B> *StoredTableScan<B>::readStoredTable(string table_name, const vector<int> & col_ordinals, const int & limit) {
    ...
    TableMetadata md = conf.table_metadata_.at(table_name);
        ...
        else if(conf.storageModel() == StorageModel::COLUMN_STORE && conf.buffer_pool_enabled_) {
...
QueryTable<B> *StoredTableScan<B>::readSecretSharedStoredTable(string table_name, const vector<int> & col_ordinals, const int & limit) {
    ...
    CryptoManager *manager = SystemConfiguration::getInstance().crypto_manager_;
...
QueryTable<B> *StoredTableScan<B>::readSecretSharedStoredTable(string table_name, const vector<int> & col_ordinals, const int & limit) {
    ...
    int current_emp_bit_size_on_disk = (conf.party_ == 1 ? empBitSizesInPhysicalBytes::evaluator_disk_size_ : (conf.party_ == 10086 ? 1 : empBitSizesInPhysicalBytes::garbler_disk_size_));

    map<int, size_t> ordinal_offsets;
    size_t col_bytes_cnt = 0;
    ordinal_offsets[0] = 0;
    for (int i = 1; i < md.schema_.getFieldCount(); ++i) {
        col_bytes_cnt += md.schema_.getField(i - 1).size() * src_tuple_cnt * current_emp_bit_size_on_disk;
        ordinal_offsets[i] = col_bytes_cnt;
    }
    col_bytes_cnt += md.schema_.getField(md.schema_.getFieldCount() - 1).size() * src_tuple_cnt * current_emp_bit_size_on_disk;
    ...
    for(auto src_ordinal : col_ordinals) {
        ...
        size_t read_size_bits = tuple_cnt * secure_field.size();

        fseek(fp, ordinal_offsets[src_ordinal], SEEK_SET);

        for(size_t j = 0; j < read_size_bits; ++j) {
    ...
    ((OutsourcedMpcManager *) conf.crypto_manager_)->protocol_->regen_label(dst_int.bits.data(), masked_values, auth_shares, dst_bit_cnt);
...
QueryTable<B> *StoredTableScan<B>::readSecretSharedStoredTable(string table_name, const int & limit) {
    ...
    for(int i = 0; i < secure_schema.getFieldCount(); ++i) {
        ...
        file_offset += src_tuple_cnt * secure_field.size() * current_emp_bit_size_on_disk;
    }
