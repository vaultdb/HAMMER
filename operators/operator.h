...
public:
    ...
    Operator(const Operator &o) :
        ...
        operator_executed_(o.operator_executed_), output_cardinality_(o.output_cardinality_), operator_id_(o.operator_id_), cte_flag_(o.cte_flag_) {
    ...
    // Returns a canonicalized version of the operator tree (duplicates replaced)
    Operator<B>* canonicalized();
    ...
    inline string getTypeString() const {
        switch(this->getType()) {
            ...
            case OperatorType::UNION:
                return "Union";
            case OperatorType::MULTIPLE_UNION:
            ...
            case OperatorType::MPC_HOSTING:
                return "MpcHostingOperator";
            default:
                throw;
        }
    }
    ...
    void setCteFlag(bool val) { cte_flag_ = val; }
    bool hasCte() const { return cte_flag_; }
    void detectCTE();
...
private:
    ...
    // Helper that does the recursion using a map to track visited operators.
    Operator<B>* canonicalizedHelper(std::unordered_map<int, Operator<B>*>& canonicalMap);
    int operator_id_ = -1;
