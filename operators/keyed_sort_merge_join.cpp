...
void KeyedSortMergeJoin<B>::setup() {
    ...
    if(is_secure_) {
        int card_bits = SystemConfiguration::getInstance().bitPackingEnabled()
            ?  ceil(log2(max_intermediate_cardinality_)) + 1 // + 1 for sign bit
            : TypeUtilities::getTypeSize(int_field_type_);
...
QueryTable<B> *KeyedSortMergeJoin<B>::runSelf() {
    ...
    for(int i = 0; i < foreign_key_cardinality_; i++) {
        ...
        for (int j = 0; j < out_schema.getFieldCount(); j++) {
            QueryFieldDesc desc = out_schema.getField(j);
            Field<B> zero = FieldFactory<B>::getZero(desc.getType());

            if (desc.bitPacked()) {
                // it is an emp::Int
                Integer tmp(desc.size()+1, 0); // +1 for sign bit
                zero = Field<B>(desc.getType(), tmp);
            }
            if (desc.getType() == FieldType::STRING) {
                string zero_s = std::string(desc.getStringLength(), '\0');
                zero = Field<B>(desc.getType(), zero_s);
            }
            if (desc.getType() == FieldType::SECURE_STRING) {
                Integer s(desc.size(), 0);
                zero = Field<B>(desc.getType(), s);
            }
            Field<B> to_write = Field<B>::If(dummy, zero, this->output_->getField(i, j));
            this->output_->setField(i, j, to_write);
        }
    }
...
QueryTable<B> *KeyedSortMergeJoin<B>::unionAndMergeTables() {
    ...
    sorter.setOperatorId(-2);
    int counter;
    sorter.bitonicMerge(unioned, sorter.getSortOrder(), 0, unioned->tuple_cnt_, true, counter);
...
QueryTable<B> *KeyedSortMergeJoin<B>::unionTables(QueryTable<B> *lhs, QueryTable<B> *rhs, const QuerySchema & dst_schema) {
    ...
    int cursor = 0;
    // merging in bitonic order so we can do a merge sort
    // need to do this one row at a time to project to homogenized schema
...
void KeyedSortMergeJoin<B>::initializeAlphas(QueryTable<B> *dst) {
    ...
    // alpha 2 = count of PK, should always be 1
...
QueryTable<B> *KeyedSortMergeJoin<B>::distribute(QueryTable<B> *input, size_t target_size) {
    ...
    // cout << "Initializing table idx to: " << table_idx.reveal(QueryFieldDesc(0, "anon", "", FieldType::BOOL, 0)) << endl;
...
QueryTable<B> *KeyedSortMergeJoin<B>::expand(QueryTable<B> *input) {
    ...
    // Field<B> suspect = dst_table->getField(0, table_id_idx_);
    // cout << "Suspect directly from table: " << suspect.reveal(QueryFieldDesc(0, "anon", "", FieldType::BOOL, 0)) << endl;
    //
    // cout << "Expanding on table with schema: " << schema << endl;
