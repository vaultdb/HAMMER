...
Operator<B> *PlanOptimizer<B>::optimizeTree() {
    ...
    // Optimize from the leaf upward
    optimizeTreeHelper(leaf);
    std::cout << "Total Plans : " << std::to_string(total_plan_cnt_) << std::endl;

    // Collect nodes used in min_cost_plan_ to avoid deleting them
    std::unordered_set<Operator<B>*> keep_alive;
    std::function<void(Operator<B>*)> collectUsed = [&](Operator<B>* node) {
        if (!node || keep_alive.count(node)) return;
        keep_alive.insert(node);
        collectUsed(node->getChild(0));
        collectUsed(node->getChild(1));
    };
    collectUsed(min_cost_plan_);

    delete new_root;

    // Safely delete operators_
    for (auto& pair : operators_) {
        if (keep_alive.count(pair.second) == 0) {
    ...
    // Safely delete support_ops_
    for (auto& op : support_ops_) {
        if (keep_alive.count(op) == 0) {
            op->setChild(nullptr);
    ...
    std::function<Operator<B>*(Operator<B>*)> stripUselessSorts;
    stripUselessSorts = [&](Operator<B>* node) -> Operator<B>* {
        if (!node) return nullptr;

        Operator<B>* lhs = node->getChild(0);
        Operator<B>* rhs = node->getChild(1);

        lhs = stripUselessSorts(lhs);
        rhs = stripUselessSorts(rhs);

        node->setChild(lhs, 0);
        node->setChild(rhs, 1);

        // Check and remove empty-collation sort
        if (node->getType() == OperatorType::SORT &&
            node->getSortOrder().empty()) {
            Operator<B>* child = lhs;  // lhs_child_ is the one used in SORT
            if (child) child->removeParent(node);

            // Prevent deleting a node that's also in keep_alive
            if (keep_alive.count(node) == 0) {
    ...
    min_cost_plan_ = stripUselessSorts(min_cost_plan_);
...
void PlanOptimizer<B>::recurseNode(Operator<B> *op) {
    ...
    // at the root node
    if(parent == nullptr) {
        // Now that CTE flag is possibly set by printTree, check and synchronize
        if (op->hasCte()) {
            synchronizeSharedInputs(op);
        }
...
template<typename B>
void PlanOptimizer<B>::synchronizeSharedInputs(Operator<B>* plan_root) {
    std::unordered_map<int, SortDefinition> seen_collations;

    std::function<void(Operator<B>*)> dfs = [&](Operator<B>* node) {
        if (!node) return;
        int id = node->getOperatorId();
        auto sort = node->getSortOrder();

        if (seen_collations.count(id)) {
            if (seen_collations[id] != sort) {
                // Insert Sort to match seen_collation
                node->setSortOrder(seen_collations[id]);
            }
        } else {
            seen_collations[id] = sort;
        }
